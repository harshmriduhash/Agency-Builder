...
export const saveActivityLogsNotification = async ({
  ...
  if (!foundAgencyId) {
    if (!subaccountId) {
      throw new Error("You need to provide at least an agencyId or subaccountId")
    }
...
export const upsertSubAccount = async (subAccount: SubAccount) => {
  try {
    ...
    if (!agencyOwner) throw console.log("Error!! No Agency Owner. Could not create subaccount");
...
export const getFunnelPageDetails = async (funnelPageId: string) => {
  const response = db.funnelPage.findUnique({
    where: { id: funnelPageId }
  });
  return response;
}
