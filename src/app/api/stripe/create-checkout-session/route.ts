...
export async function POST(req: Request) {
  ...
  if (!subAccountConnectAccId || !prices.length)
    return NextResponse.json({ error: 'Stripe Account Id or Price Id is missing' });
  ...
  if (
    ...
    return NextResponse.json({ error: 'Internal server configurations error' });

    // No need unless we want to send payments to the account.
    // TODO: Send or transfer money to a connected account
    ...
    const session = await stripe.checkout.sessions.create(
      {
        ...
        ...(!subscriptionPriceExists && {
          payment_intent_data: {
            ...
            application_fee_amount: + process.env.NEXT_PUBLIC_PLATFORM_ONETIME_FEE,
          },
        }),
