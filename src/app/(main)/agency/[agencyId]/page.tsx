import AreaChartCustom from "@/components/global/area-chart-custom";
...
const Page = async ({
  ...
  if (agencyDetails.connectAccountId) {
    ...
    closingRate = +(
      (totalClosedSessions.length / (checkoutSessions.data.length || 1)) *
      100
    ).toFixed(2)
  }
  ...
  return (
    <div className="relative  h-full">
      {!agencyDetails?.connectAccountId && (
        <div className="absolute -top-10  -left-10 right-0 bottom-0 z-30 flex justify-center items-center backdrop-blur-md bg-background/50">
          <Card>
            <CardHeader>
              <CardTitle>Connect Your Stripe</CardTitle>
              <CardDescription>
                You need to connect your stripe account to see matrics.
              </CardDescription>
              <Link
        ...
        <div className="flex gap-4 flex-col xl:!flex-row">
          <Card className="flex-1 relative">
            <CardHeader>
              ...
              <CardTitle className="text-4xl">
                {net ? `${currency} ${net.toFixed(2)}` : `${currency} 0.00`}
              </CardTitle>
          ...
          <Card className="flex-1 relative">
            <CardHeader>
              ...
              <CardTitle className="text-4xl">
                {potentialIncome ? `${currency} ${potentialIncome.toFixed(2)}` : `${currency} 0.00`}
              </CardTitle>
              <small className="text-xs text-muted-foreground">
                For the year {currentYear}
              </small>
          ...
          <Card className="flex-1 relative">
            ...
            <CardFooter>
              <div className="flex flex-col w-full">
                ...
                <Progress
                  value={(subaccounts.length / (agencyDetails?.goal || 1)) * 100}
                />
              </div>
            </CardFooter>
